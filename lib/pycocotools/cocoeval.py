...
class COCOeval:
        ...
        if type(obj['segmentation']) == list:
          ...
          if len(obj['segmentation']) == 1:
            ...
            obj['segmentation'] = mask.merge(obj['segmentation'])
              ...
              type(obj['segmentation']['counts']) == list):
          obj['segmentation'] = mask.frPyObjects(
              [obj['segmentation']], t['height'], t['width'])[0]
        elif (type(obj['segmentation']) == dict and
              type(obj['segmentation']['counts'] == unicode or
                   type(obj['segmentation']['counts']) == str)):
          pass
        else:
  ...
  def evaluate(self):
    ...
    store results (a list of dict) in self.evalImgs
    :return: None
    '''
    tic = time.time()
  ...
  def evaluateImg(self, imgId, catId, aRng, maxDet):
    ...
    gtind = [ind for (ind, g) in sorted(enumerate(gt),
                                        key=lambda ind, g: g['_ignore'])]
    ...
    ious = self.ious[imgId, catId][0:maxDet, np.array(
        gtind)] if N_iou > 0 else self.ious[imgId, catId]
  ...
  def summarize(self):
    ...
    def _summarize(ap=1, iouThr=None, areaRng='all', maxDets=100):
      ...
      aind = [i for i, aRng in enumerate(
          ['all', 'small', 'medium', 'large']) if aRng == areaRng]
      ...
      print(iStr.format(titleStr, typeStr, iouStr,
                        areaStr, maxDetsStr, '%.3f' % (float(mean_s))))
      return mean_s
