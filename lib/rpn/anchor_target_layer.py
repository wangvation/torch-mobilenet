...
from .bbox_transform import bbox_transform_batch
...
class _AnchorTargetLayer(nn.Module):
  ...
  def __init__(self, feat_stride, scales, ratios):
    ...
    np_anchors = generate_anchors(scales=np.array(anchor_scales),
                                  ratios=np.array(ratios))
    self._anchors = torch.from_numpy(np_anchors).float()
  ...
  def forward(self, input):
    ...
    for i in range(batch_size):
      ...
      if sum_fg[i] > num_fg:
        ...
        # torch.randperm seems has a bug on multi-gpu
        # setting that cause the segfault.
